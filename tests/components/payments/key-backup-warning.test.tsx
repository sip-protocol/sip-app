...
describe("KeyBackupWarning", () => {
  ...
  it("renders warning message", () => {
    render(
      <KeyBackupWarning onConfirm={mockOnConfirm} onCancel={mockOnCancel} />
    )
  ...
  it("displays backup instructions", () => {
    ...
    expect(
      screen.getByText(/Keys are stored in your browser/i)
    ).toBeInTheDocument()
    expect(
      screen.getByText(/Clearing browser data will delete your keys/i)
    ).toBeInTheDocument()
  })
  ...
  it("shows warning about lost keys", () => {
    ...
    expect(
      screen.getByText(/There is no way to recover lost keys/i)
    ).toBeInTheDocument()
  })
