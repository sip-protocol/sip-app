...
describe("useStealthFund", () => {
  ...
  it("returns null fund initially", () => {
    const { result } = renderHook(() => useStealthFund())
    expect(result.current.lastFund).toBeNull()
    expect(result.current.tx.status).toBe("idle")
  })

  it("sendFund returns null when wallet not connected", async () => {
    const { result } = renderHook(() => useStealthFund())

    let fund: unknown = "not-null"
    await act(async () => {
      fund = await result.current.sendFund("proj-1", "micro")
    })

    expect(fund).toBeNull()
  })

  it("exposes sendFund and tx properties", () => {
    const { result } = renderHook(() => useStealthFund())
    expect(typeof result.current.sendFund).toBe("function")
