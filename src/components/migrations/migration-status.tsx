...
const STEPS: StepConfig[] = [
  {
    id: "scanning_wallet",
    label: "Scan Wallet",
    description: "Scanning for tokens in source protocol",
    icon: "\uD83D\uDD0D",
  },
  {
    id: "generating_stealth",
    label: "Generate Stealth Address",
    description: "Creating unlinkable destination for Sunrise deposit",
    icon: "\uD83D\uDD10",
  },
  {
    id: "withdrawing_from_source",
    label: "Withdraw from Source",
    description: "Withdrawing SOL from dead protocol",
    icon: "\uD83D\uDCE4",
  },
  {
    id: "depositing_to_sunrise",
    label: "Deposit to Sunrise",
    description: "Depositing SOL to Sunrise Stake for gSOL + carbon offsets",
    icon: "\uD83C\uDF31",
  },
  {
    id: "complete",
  ...
  return (
    <div
      className={cn(
        ...
        isComplete && "bg-green-900/20 border-green-800",
        !isFailed && !isComplete && "bg-green-900/10 border-green-800/50",
        className
      )}
    >
      {/* Header */}
      <div className="px-4 pt-4 pb-2">
        <p
          className={cn(
            ...
            !isFailed && !isComplete && "text-green-300"
          )}
        >
          {isFailed
            ? "Migration Failed"
          ...
          {STEPS.map((step, index) => {
            ...
            return (
              ...
              >
                  ...
                  {index < STEPS.length - 1 && (
                    <div
                      className={cn(
                        ...
                        isStepComplete ? "bg-green-500/40" : "bg-gray-700"
                      )}
                    />
                  )}
                </div>
                ...
                {/* Active spinner */}
                {isCurrent && (
                  <LoadingSpinner className="w-4 h-4 text-green-400 ml-auto flex-shrink-0 mt-1" />
                )}
              </div>
            )
          })}
