...
interface MetaverseStatusProps {
  currentStep: MetaverseStep
  mode: "explore" | "teleport"
  error?: string | null
  className?: string
}
...
const TELEPORT_STEPS: StepConfig[] = [
  {
    id: "generating_proof",
...
export function MetaverseStatus({
  currentStep,
  mode,
  error,
  className,
}: MetaverseStatusProps) {
  ...
  const finalSteps: MetaverseStep[] = ["entered", "arrived"]
  ...
  return (
    <div
      className={cn(
        ...
        !isFailed && !isComplete && "bg-indigo-900/10 border-indigo-800/50",
        className
      )}
    >
      {/* Header */}
      <div className="px-4 pt-4 pb-2">
        <p
          className={cn(
            ...
            !isFailed && !isComplete && "text-indigo-300"
          )}
        >
          {isFailed
            ? header.failed
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-center flex-shrink-0">
                  <div
                    className={cn(
                      ...
                      isCurrent &&
                        "bg-indigo-500/20 text-indigo-300 ring-2 ring-indigo-500/40 animate-pulse",
                ...
                {isCurrent && !isComplete && (
                  <LoadingSpinner className="w-4 h-4 text-indigo-400 ml-auto flex-shrink-0 mt-1" />
                )}
              </div>
            )
          })}
